let getvalue = (id) => {
  ...
  let fetchExchangeRate = async () => {
    try {
        ...
        const rateEUR = data.rates.EUR; // Get the rate for GBP to NGN
        return rateEUR;
    } catch (error) {
  ...
  let updateNisabDisplay = async () => {
    ...
    const rateEUR = await fetchExchangeRate();
    const nisabEUR = (nisabGBP * rateEUR).toFixed(2); // Convert to NGN
    document.querySelector("p").innerHTML = `Today's Nisab is: €${nisabEUR}`;
  };
  ...
  let calculate = () => {
    ...
    if (amt_eligible > 0) {
        ...
        donateButton.href = "/donate.html"; // Direct to donation page
